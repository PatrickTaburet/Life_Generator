...
export type Props = {
    ...
    "Max distance interaction": number;
    Scale: number;
    Reset: () => void;
    Random: () => void;
    drawingMode: () => void;
...
export function setupGUI(){
    ...
    colors.forEach(color => {
        ...
        // Color title
        const folderTitle = (colorFolders[color] as any).__ul.querySelector('.dg .title');
...
function addStyledButton(folder: dat.GUI, propName: string, id: string, buttonText: string | null = null, defaultColor = '#21bbe6'){
...
function updateButtonColor(buttonId: string, boolVariable: boolean, mainColor: string = 'red', secondColor: string = '#21bbe6' ) {
    ...
    const Button: HTMLElement = element.parentElement;
...
function updateSliderConstraints() {
    colors.forEach(color => {
        const colorParticleController: dat.GUIController | undefined  = guiMain.__folders["Particles Rules"].__folders[color].__controllers[0];
    ...
    const maxDistanceController: dat.GUIController | undefined = guiMain.__folders["Global Settings"].__controllers.find(ctrl => ctrl.property === 'Max distance interaction');
    ...
    const scaleController: dat.GUIController | undefined  = guiMain.__folders["Global Settings"].__controllers.find(ctrl => ctrl.property === 'Scale');
    if (scaleController) {
...
export function openGuiFolers(){    
    ...
    if (checkbox instanceof HTMLInputElement) checkbox.checked =  appStates.isMobile ? false : true;
}
