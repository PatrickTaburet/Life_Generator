...
let sketch = (p) => {
    ...
    p.draw = () => {
        ...
        colors.forEach(color1 => {
            if (!props[`${color1} Visible`])
                return;
            colors.forEach(color2 => {
                if (!props[`${color2} Visible`])
                    return;
                applyRules(particles.filter(particle => particle.color === color1), particles.filter(particle => particle.color === color2), props[`${color1[0]} <--> ${color2[0]}`], p);
                appStates.isNexus && (particles.filter(particle => particle.color === color1).forEach(a => {
